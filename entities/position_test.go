...
// var (
// 	B100e6  = int256.NewInt(100e6)
// 	B100e12 = int256.NewInt(100e12)
...
// 	tickSpacing := constants.TickSpacings[constants.Fee004]
// 	p, err := NewPool(DAI, USDC, constants.Fee004, poolSqrtRatioStart, int256.NewInt(0), int256.NewInt(0), poolTickCurrent, nil)
// 	if err != nil {
// 		panic(err)
// 	}
...
// 	p, err := NewPosition(DAIUSDCPool, int256.NewInt(1), -8, 8)
// 	assert.NoError(t, err)
// 	assert.Equal(t, int256.NewInt(1), p.Liquidity)

// 	// can use min and max ticks
// 	p, err = NewPosition(DAIUSDCPool, int256.NewInt(1), NearestUsableTick(utils.MinTick, tickSpacing), 8)
// 	assert.NoError(t, err)
// 	assert.Equal(t, int256.NewInt(1), p.Liquidity)

// 	// tick lower must be less than tick upper
// 	_, err = NewPosition(DAIUSDCPool, int256.NewInt(1), 10, -10)
...
// 	_, err = NewPosition(DAIUSDCPool, int256.NewInt(1), -5, 10)
// 	assert.ErrorIs(t, err, ErrTickLower)

// 	// tick lower must be greater than MIN_TICK
// 	_, err = NewPosition(DAIUSDCPool, int256.NewInt(1), NearestUsableTick(utils.MinTick, tickSpacing)-tickSpacing, 10)
// 	assert.ErrorIs(t, err, ErrTickLower)

// 	// tick upper must be multiple of tick spacing
// 	_, err = NewPosition(DAIUSDCPool, int256.NewInt(1), -8, 17)
// 	assert.ErrorIs(t, err, ErrTickUpper)

// 	// tick upper must be less than MAX_TICK
// 	_, err = NewPosition(DAIUSDCPool, int256.NewInt(1), -8, NearestUsableTick(utils.MaxTick, tickSpacing)+tickSpacing)
// 	assert.ErrorIs(t, err, ErrTickUpper)
// }
...
// 	DAIUSDCPool, poolTickCurrent, tickSpacing := initPool()
// 	// 0 slippage
// 	slippageTolerance := entities.NewPercent(int256.NewInt(0), int256.NewInt(1))
...
// 	// 0.05% slippage
// 	slippageTolerance = entities.NewPercent(int256.NewInt(5), int256.NewInt(10000))
...
// 	slippageTolerance = entities.NewPercent(int256.NewInt(5), int256.NewInt(100))

// 	// is correct for pool at min price
// 	minPricePool, err := NewPool(DAI, USDC, constants.Fee001, utils.MinSqrtRatio, int256.NewInt(0), int256.NewInt(0), utils.MinTick, nil)
// 	assert.NoError(t, err)
...
// 	maxPricePool, err := NewPool(DAI, USDC, constants.Fee001, int256.New().Sub(utils.MaxSqrtRatio, int256.NewInt(1)), int256.NewInt(0), int256.NewInt(0), utils.MaxTick-1, nil)
// 	assert.NoError(t, err)
...
// 	slippageTolerance := entities.NewPercent(int256.NewInt(0), int256.NewInt(100))
